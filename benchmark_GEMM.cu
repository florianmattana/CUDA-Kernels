...
int main()
{
    ...
    std::cout << "Version 4.0 : padded and vecto ( bench + reference) " << std::endl;
    ...
    //============================== VALIDATION: vecto and padded ==============

    std::cout << "STEP 8 : validate vectorized and padded kernel  ..." << std::endl;

    CC(cudaMemset(d_matrix_C, 0, bytes_C));
    vecto_and_padded <TM,TN> <<<blocks_4, threads_4 >> > (d_matrix_A, d_matrix_B, d_matrix_C, M, K, N);
    CC(cudaGetLastError());
    CC(cudaDeviceSynchronize()); // debug

    CC(cudaMemcpy(matrix_C_gpu.data, d_matrix_C, bytes_C, cudaMemcpyDeviceToHost));

    std::cout << "[VECTO/PADDED TILED] ";
    validation(matrix_C_cpu.data, matrix_C_gpu.data, M, N, tol_r, tol_a);
